...
# Entrada de demandas
num_produtos = st.number_input("Número de produtos", min_value=1, value=1, step=1)

demands = []
for i in range(num_produtos):
    product = st.selectbox(f"Produto {i+1}", list(produtos.keys()), key=f"produto_{i}")
    weight = st.number_input(f"Peso do Produto {i+1} (kg)", min_value=1, value=10000, step=1000, key=f"peso_{i}")
    if product in produtos:
        width = produtos[product]
        demands.append({"width": width, "weight": weight})
    else:
        st.error(f"Produto desconhecido: {product}")
        st.stop()
